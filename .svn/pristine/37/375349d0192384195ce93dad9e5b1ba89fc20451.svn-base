    ...
    <div class="observer-list">
      <ul>
        <li v-for="(item,index) in observerList" :key="index">
          <div class="ad-box">
            <div class="selection" :class="{'on':item.checked}" @click="choseSelect(item.id,index)"></div>
            <div class="lf">
              <div class="user-info">
...
export default {
  ...
  methods: {
    choseSelect(id, index) {
      ...
      this.observerList[index].checked = true;
      localStorage.setItem(
        "realInfo",
        JSON.stringify(this.observerList[index])
      );
      this.$router.back();
    },
    //获取地址列表
    ...
    //去新增地址
    toAdd() {
      this.$router.push("/observerInfoChose");
    },
    //编辑地址
    toEdit() {
      this.$router.push({
